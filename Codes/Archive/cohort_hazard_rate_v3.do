...
Purpose: Hazard rate - including pending accounts and taking cases with > 365 days as open
Author: Hamza Syed
******************************************************************/
...
replace case_days_med = 400 if case_days_med > 400
...
replace closed = 0 if case_days_med > 365
...
bytwoway line prop_open case_days_med, by(year) bgcolor(white) xtitle("case days") ytitle("proportion of open cases for the year") note("Note: closed cases here mean either with agreement or without; cases with >365 days marked open") legend(rows(2)) 
graph save "`path'/Output/cohort_wise_survival_with_pending_lostcause", replace
graph export "`path'/Output/cohort_wise_survival_with_pending_lostcause.png", as(png) replace

//Hazard analysis
stset case_days_med, fail(open)
sts list, by(year) cumhaz
sts graph, by(year) cumhaz legend(rows(3)) bgcolor(white) note("Note: closed cases here mean either with agreement or without; cases with >365 days marked open")
graph save "`path'/Output/cohort_wise_hazard_with_pending_lostcause", replace
graph export "`path'/Output/cohort_wise_hazard_with_pending_lostcause.png", as(png) replace
