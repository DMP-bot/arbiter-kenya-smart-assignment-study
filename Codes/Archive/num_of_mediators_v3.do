...
preserve
tab casetype
keep if ref_year == 2022
drop if ref_month == 11
tab ref_month_year
bys ref_month_year: gen cases = _N
duplicates drop ref_month_year, force
summ cases
graph twoway bar cases ref_month_year, graphregion(color(white)) xtitle("month") ytitle("number of cases") title("number of cases referred per month")  note("data till 5th October 2022")
...
graph save "`path'/Output/relevant_cases_monthly2_`current_date'", replace
graph export "`path'/Output/relevant_cases_monthly2_`current_date'.png", as(png) replace
restore
...
//Importing data
use "`path'/Data/cases_cleaned_pull05Oct2022.dta", clear
