...
*import delimited "`path'\Data\cases_raw.csv", clear
import delimited "`path'\Data\de_identified_cases_with_appointers_2023-02-27T17_08_37.839808Z.csv", clear 

// New (Didac): In the new data download the names of the variables was 
// different, it is adjusted here
rename (id referraldate casenumber originalcasenumber valuemode casevalue casestatus outcomename agreementmode ///
caseoutcomeagreement pendingreason mediatorappointmentdate conclusiondate forwardedforpaymentdate ///
casedays mediatorappointmentdays numberofplaintifflanguages numberofdefendantlanguages casetype ///
courtdivision courttype courtstation inferredcourt mediatorid referralmode sessiontype ///
completioncertificatedate createdat updatedat appointmentdataentrytimestamp appointedatcasecreationtime ///
appointeruserid oldmediatormac newmediatormac defendantlanguages plaintifflanguages) ///
(id referral_date case_number original_case_number value_mode case_value case_status outcome_name agreement_mode ///
case_outcome_agreement pending_reason mediator_appointment_date conclusion_date forwarded_for_payment_date ///
case_days mediator_appointment_days number_of_plaintiff_languages number_of_defendant_languages case_type ///
court_division court_type court_station inferred_court mediator_id referral_mode session_type ///
completion_certificate_date created_at updated_at appointment_data_entry_timestamp appointed_at_case_creation_time ///
appointer_user_id old_mediator_mac new_mediator_mac defendant_languages plaintiff_languages)
...
//Date variables
	...
	*Creation date in cadaster/arbiter - Review
...
//Selecting usable cases
	...
	*Outsheeting issue cases
		...
		export excel using "`path'/Data/issues.xlsx", firstrow(variables) replace
		restore
...
//Saving cleaned file
save "`path'/Data/cases_cleaned.dta", replace
